...
export const routes: Routes = [
...

  {
    path: '',
    loadComponent: () =>
      import('./app/layout/app.layout').then(m => m.AppLayout),
    children: [
  ...
  // { path: 'inspectora/generar-multa', loadChildren: () => import('./app/features/inspectora/generar-multa.routes').then(m => m.GENERAR_MULTA_ROUTES) },
  { path: 'permisos', loadChildren: () => import('./app/features/admin/permissions/permisos-page.routes').then(m => m.PERMISOS_ROUTES) },
    ]
  },

  // { path: 'contenido-documento', loadChildren: () => import('./app/features/auth/pages/pagina-Inicio/inicio/inicio.routes').then(m => m.INICIO_ROUTES) },

  { path: 'identificacion', loadChildren: () => import('./app/features/auth/pages/identificacion/identificacion.routes').then(m => m.IDENTIFICACION_ROUTES) },

  // Módulo Auth (aquí vive /auth/inicio)
  { path: 'auth', loadChildren: () => import('./app/features/auth/auth.routes').then(m => m.AUTH_ROUTES) },
