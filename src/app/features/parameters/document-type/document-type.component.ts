...
export class DocumentTypeComponent implements OnInit {
  ...
  showConfirm = false;
  documentTypeAEliminar: DocumentType | null = null;
  documentTypeSeleccionado: DocumentType | null = null;
  ...
  private cargarDocumentTypes(): void {
    ...
    this.service.genericService.getAll<DocumentType>(this.service.endpoint, 'GetAll')
  ...
  crearDocumentType(): void {
    if (this.documentTypeForm.valid) {
      ...
      this.service.genericService.create<DocumentType>(this.service.endpoint, documentTypeData)
        .pipe(finalize(() => this.loading = false))
        .subscribe({
          next: (nuevoDocumentType: DocumentType) => {
            this.successMsg = 'Tipo de documento creado exitosamente.';
            this.cargarDocumentTypes(); // Recargar la lista
  ...
  abrirFormularioActualizar(documentType: DocumentType): void {
  ...
  actualizarDocumentType(): void {
    if (this.updateForm.valid && this.documentTypeSeleccionado && this.documentTypeSeleccionado.id) {
      ...
      this.service.genericService.update<DocumentType>(this.service.endpoint, this.documentTypeSeleccionado.id, documentTypeActualizado)
        .pipe(finalize(() => this.loading = false))
        .subscribe({
          next: (documentTypeActualizado: DocumentType) => {
  ...
  confirmarEliminacion(documentType: DocumentType): void {
